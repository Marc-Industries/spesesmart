...
import { 
  ...
  BrainCircuit, X, Settings, ArrowLeft, Loader2, Moon, Sun, Globe
} from 'lucide-react';
import { Transaction, User, TransactionType, Language } from './types';
...
function App() {
  ...
  const [uiLanguage, setUiLanguage] = useState<Language>('it'); // For login screen only
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (selectedLoginUser && passwordInput === selectedLoginUser.password) {
      ...
      // Sync UI language with user preference
  ...
  // Stats Calculation
  const stats = useMemo(() => {
    if (!currentUser) return { income: 0, expense: 0, balance: 0 };
    const base = currentUser.preferences.currency;
    const income = transactions.filter(t => t.type === TransactionType.INCOME).reduce((acc, t) => acc + convertCurrency(t.amount, t.currency, base), 0);
    const expense = transactions.filter(t => t.type === TransactionType.EXPENSE).reduce((acc, t) => acc + convertCurrency(t.amount, t.currency, base), 0);
             ...
             <div className="flex gap-2">
               {(['it', 'en', 'pl'] as Language[]).map(l => (
                  <button 
                    ...
                    className={`text-xs font-bold px-2 py-1 rounded-md uppercase ${uiLanguage === l ? 'bg-blue-600 text-white' : 'text-slate-400'}`}
                  >
                    {l}
                  </button>
               ))}
          ...
          {loginStep === 'select_user' ? (
              ...
              <div className="space-y-3">
                {MOCK_USERS.map(user => (
                  <button 
                    ...
                    className={`w-full flex items-center gap-4 p-3 rounded-xl border transition-all group ${isDarkMode ? 'border-slate-700 hover:bg-slate-700 hover:border-blue-500' : 'border-slate-200 hover:bg-blue-50 hover:border-blue-500'}`}
                  >
            ...
            <>
              <div className="flex items-center justify-start mb-6">
                <button onClick={handleBackToUserSelect} className="text-slate-400 hover:text-slate-600 flex items-center gap-1 text-sm">
                   <ArrowLeft className="w-4 h-4" /> {t('back', uiLanguage)}
                </button>
              </div>
              ...
              <form onSubmit={handlePasswordSubmit} className="space-y-4">
                 <div>
                   <input 
                    ...
                    className={`w-full p-3 border rounded-xl outline-none transition-all ${isDarkMode ? 'bg-slate-900 border-slate-700 text-white focus:border-blue-500' : 'bg-slate-50 text-slate-900 border-slate-200 focus:border-blue-500'} ${loginError ? 'border-red-500' : ''}`}
                    placeholder={t('password_placeholder', uiLanguage)}
                   />
                   {loginError && <p className="text-red-500 text-xs mt-1 text-left">{t('wrong_password', uiLanguage)}</p>}
                 </div>
                 <button type="submit" className="w-full bg-blue-600 text-white py-3 rounded-xl font-semibold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-600/20">
                    {t('login_btn', uiLanguage)}
                 </button>
              </form>
            </>
          )}
    ...
    <div className={`min-h-screen pb-20 md:pb-0 flex transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <aside className={`hidden md:flex flex-col w-64 border-r h-screen fixed top-0 left-0 z-10 transition-colors ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
        ...
        <div className={`p-4 border-t ${isDarkMode ? 'border-slate-700' : 'border-slate-100'}`}>
           <div className="flex items-center justify-between mb-4 px-2">
              ...
              <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-1.5 rounded-lg transition-colors ${isDarkMode ? 'bg-slate-700 text-yellow-400' : 'bg-slate-100 text-slate-400'}`}>
           ...
           <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 text-slate-500 hover:text-red-500 text-sm py-2">
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 max-w-7xl mx-auto w-full">
        ...
        <div className="md:hidden flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
             <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2 rounded-full border ${isDarkMode ? 'bg-slate-800 border-slate-700 text-yellow-400' : 'bg-white border-slate-200 text-slate-600'}`}>
                {isDarkMode ? <Sun className="w-5 h-5"/> : <Moon className="w-5 h-5"/>}
             </button>
        ...
        {/* LOADING INDICATOR */}
        {isLoadingData && (
          <div className="fixed top-0 left-0 w-full h-1 bg-blue-900/20 overflow-hidden z-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className={`p-6 rounded-2xl border shadow-sm flex items-center justify-between ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
            <div>
              <p className="text-slate-400 text-sm font-medium mb-1">{t('income', language)} <span className="text-xs">{t('monthly', language)}</span></p>
              <h2 className="text-2xl font-bold text-emerald-500">+{formatCurrency(stats.income, baseCurrency)}</h2>
            </div>
            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-emerald-500 ${isDarkMode ? 'bg-emerald-500/10' : 'bg-emerald-100'}`}>
          ...
          <div className={`p-6 rounded-2xl border shadow-sm flex items-center justify-between ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
            <div>
              <p className="text-slate-400 text-sm font-medium mb-1">{t('expense', language)} <span className="text-xs">{t('monthly', language)}</span></p>
              <h2 className="text-2xl font-bold text-red-500">-{formatCurrency(stats.expense, baseCurrency)}</h2>
            </div>
            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-red-500 ${isDarkMode ? 'bg-red-500/10' : 'bg-red-100'}`}>
        ...
        <div className="mb-6">
          {!aiAnalysis ? (
            <button 
              ...
              className="w-full md:w-auto bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 rounded-xl shadow-lg shadow-blue-500/20 font-medium flex items-center justify-center gap-2 hover:opacity-90 transition-opacity"
            >
              {isAnalyzing ? (
                 <>{t('analyzing', language)}</>
              ) : (
                 <><BrainCircuit className="w-5 h-5" /> {t('ai_analyze', language)}</>
              )}
            </button>
          ) : (
            <div className={`p-6 rounded-2xl border relative animate-fade-in ${isDarkMode ? 'bg-indigo-900/20 border-indigo-800' : 'bg-gradient-to-br from-indigo-50 to-purple-50 border-indigo-100'}`}>
              ...
              <div className="flex items-start gap-3">
                 <div className={`p-2 rounded-lg shadow-sm text-indigo-500 ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <BalanceTrendChart transactions={transactions} baseCurrency={baseCurrency} language={language} isDarkMode={isDarkMode} />
              <ExpensePieChart transactions={transactions} baseCurrency={baseCurrency} language={language} isDarkMode={isDarkMode} />
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
               <CategoryBarChart transactions={transactions} baseCurrency={baseCurrency} language={language} isDarkMode={isDarkMode} />
               {/* Placeholder for future widgets */}
               <div className={`rounded-xl border p-4 flex items-center justify-center text-slate-400 ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
                  Coming Soon...
               </div>
            </div>
            ...
            <div>
              ...
              <TransactionList transactions={transactions.slice(0, 5)} onDelete={handleDeleteTransaction} baseCurrency={baseCurrency} language={language} isDarkMode={isDarkMode} />
            </div>
          </div>
        )}
      ...
      <div className={`md:hidden fixed bottom-0 left-0 w-full border-t flex justify-around p-3 z-40 pb-safe transition-colors ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
        ...
        <button onClick={() => setShowAddModal(true)} className="bg-blue-600 text-white rounded-full p-4 -mt-8 shadow-lg shadow-blue-600/30">
      ...
      <button 
        ...
        className="hidden md:flex fixed bottom-10 right-10 bg-blue-600 text-white p-4 rounded-full shadow-2xl shadow-blue-600/40 hover:scale-105 transition-transform z-40 items-center gap-2 pr-6"
      >
