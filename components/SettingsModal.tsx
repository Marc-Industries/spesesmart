...
import { X, Check, Lock, User as UserIcon, Image as ImageIcon } from 'lucide-react';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ user, onUpdate, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-fade-in max-h-[90vh] flex flex-col">
        <div className="p-6 bg-slate-900 text-white flex justify-between items-center flex-shrink-0">
          <div className="font-bold text-lg">Impostazioni Account</div>
        ...
        <div className="overflow-y-auto no-scrollbar flex-1 p-6">
            <form onSubmit={handleSubmitProfile} className="space-y-8">
              ...
              <div className="flex gap-3 pt-6 border-t dark:border-slate-800">
                <button type="button" onClick={onClose} className="flex-1 py-3 text-sm font-black text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-2xl transition-colors">{t('cancel', language).toUpperCase()}</button>
                <button type="submit" className="flex-1 py-3 bg-blue-600 text-white text-sm font-black rounded-2xl hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/30">{t('save', language).toUpperCase()}</button>
              </div>
            </form>
        </div>
      </div>
